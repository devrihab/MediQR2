  ...
  const onSubmit = async (data: SetupFormData) => {
    try {
      ...
      const fullPatient = {
        ...
        conditions: data.conditions.map(c => c.value).filter(Boolean),
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView style={{ flex: 1 }} contentContainerStyle={styles.container}>
        ...
        <Button title="Complete Setup" onPress={handleSubmit(onSubmit)} isLoading={loading} style={styles.button} />
      </ScrollView>
